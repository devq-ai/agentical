...
# Write summary information at the beginning
cat > "$OUTPUT_FILE" << EOL
REPOSITORY INVENTORY
==================
Generated: $(date)
Directory: $(cd "$REPO_DIR" && pwd)
Note: Shows all files including hidden ones, with sensitive files flagged
...
# ELAPSED_TIME_PLACEHOLDER

FORMAT: [LEVEL] [TYPE] [SIZE] [FILES] [PATH]
==================
...
echo "Inventory complete. Found $TOTAL_FILES files in $TOTAL_DIRS directories. Total size: $(format_size $TOTAL_SIZE)" >> "$OUTPUT_FILE"
echo "Time elapsed: ${MINUTES}m ${SECONDS}s" >> "$OUTPUT_FILE"

# Clean up temp files
rm -f "$TEMP_FILES" "$TEMP_DIRS" "$DIR_SIZES" "$DIR_SIZES.tmp" "$DIR_FULL_STATS" "$DIR_SUMMARY"

# Update the elapsed time in the output file
sed -i '' "s/# ELAPSED_TIME_PLACEHOLDER/Time elapsed: ${MINUTES}m ${SECONDS}s/" "$OUTPUT_FILE"
